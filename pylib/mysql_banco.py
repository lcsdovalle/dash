...
class banco():
    ...
    def ler(self,condicao=None,oque=None):
        what = oque if oque is not None else "*"
        how = f"where {condicao}" if condicao is not None else ""
        q = "SELECT {} FROM {} {}".format(what,self.table,how)
        self.query = str(q).strip()
        return self
