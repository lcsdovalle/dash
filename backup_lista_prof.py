...
def execute(usuario):
        ...
        for inep in usuario.get('inep').split(','):
            try:
                ...
                for atv in atividades:
                    if atv.criado.split("T")[0] in usuario.get('invervalosss'):
            ...
            a_lista.add_row_csv([
                ...
                usuario.get('ultimo_acesso'),
                usuario.get('primeiro_acesso'),
                usuario.get('acessos_ultima_semana') ,
                usuario.get('acessos_desde_inicio') ,
                total_atividades,
    ...
    try:
        ...
        f.ultimo_acesso = usuario.get('ultimo_acesso')
        f.primeiro_acesso =  usuario.get('primeiro_acesso')

        f.acessos_ultima_semana = usuario.get('acessos_ultima_semana')
        f.acessos_desde_inicio = usuario.get('acessos_desde_inicio') 
...
if __name__ == "__main__":
    ...
    for email in contabilizador:
        ...
        if os_dias:
            ...
            usuario['inep'] = usuario.get('inep')
            usuario['ultimo_acesso'] = os_dias[-1]
            usuario['primeiro_acesso'] = os_dias[0]
            usuario['acessos_ultima_semana'] = acessou_ultima_semana
            usuario['acessos_desde_inicio'] = total_acessos_desde_primeiro_login
            usuario['invervalosss'] = intervaloss

            try:
                results.append(pool.apply_async(execute,(usuario,)))
